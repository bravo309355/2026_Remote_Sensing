...
def package_submission(config: ARIAConfig, risk_json_path: Path, risk_png_path: Path) -> None:
    config.submission_dir.mkdir(parents=True, exist_ok=True)
    copy_if_exists(PROJECT_ROOT / "ARIA.ipynb", config.submission_dir / "ARIA.ipynb")
...
def run_aria_pipeline(config: ARIAConfig) -> ARIAResult:
    ...
    package_submission(config, risk_json_path, risk_png_path)
