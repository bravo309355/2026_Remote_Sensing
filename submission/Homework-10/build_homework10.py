...
本案例的 bbox `[121.28, 23.56, 121.52, 23.76]` 主要涵蓋**花蓮縱谷流域** — 馬太鞍溪上游山區、光復鄉、鳳林鎮一帶的縱谷低地。**並非沿海平原**（海岸線位於 121.6 以東，超出 bbox）。bbox 內地形主要是中央山脈東側與海岸山脈西側夾擊的縱谷地形。

#### DEM 適用性 — 以實際數據檢驗

| 觀察項 | 數值 | 含義 |
|--------|------|------|
| Slope > 25° 比例 | 52.9% of scene | 超過半數 bbox 為陡坡（縱谷兩側山區） |
| Slope filter 移除 SAR 水體 | 1.86 km² | ~等於 SAR 偵測 1.85 km² → 幾乎所有原始 SAR 水體都在陡坡上 |
| High Confidence 變化 | 0.46 → 0.11 km² | **-75% reduction** — 套用後 dual-sensor 確認區大幅縮減 |

#### 實證結果暴露的矛盾

套了 slope filter 後 High Conf 只剩 0.11 km²，但鳳凰颱風的真實災害規模在花蓮縣不可能這麼小。三種可能解釋：

1. **DEM 不準確**：上游 2025/7 薇帕颱風後的崩塌帶在舊 DEM 仍是陡溪谷，套 slope > 25° 把堰塞湖等真實水體誤刪 → **漏報**
2. **NDWI 樣本不足**：S2 post 雲量 89% 下，clear 區域過小，無法與 SAR 充分交集
3. **本 bbox 災害集中在水流走廊**：颱風淹水可能沿溪谷分布（窄帶），而非大面積開放水體 → SAR backscatter 訊號偏小

#### 替代方案（針對 DEM 不適用的山區）

當 DEM 不可信時，改用幾何形態學 + 統計特徵清理假水體：

| 方法 | 機制 | 本作業是否套用 |
|------|------|--------------|
| Morphological opening | 先侵蝕再膨脹，去掉孤立小斑點 | ✅ Task 1（3×3, 1 iter） |
| Connected component filter | 移除小於 0.5 ha 的水體碎片 | ✅ Task 1（門檻 50 px） |
| 多時相一致性（diff threshold）| 比對災前 SAR（10/30），只保留**新增**水體 | ⚠ 已載入 pre SAR 但未做差異閾值（可作為延伸） |
| NDWI 雙重確認 | 取 SAR ∩ NDWI 交集 | ✅ Task 2（High Confidence class） |
| InSAR coherence | 災前/後相位相干性下降的區域 = 表面變化 | ❌ 進階；需 SLC 資料 |

#### 結論

本作業同時套用 slope filter + morphological cleanup + NDWI 交集，屬於**保守版**處理 — **誤刪風險高、漏判風險低**。這個取捨偏向 precision over recall，與課堂 Exercise（「寬抓 + 後處理」）是相反取向。實務上應**同時呈現前/後地形校正兩個版本**讓決策者判斷，而非只看校正後結果。"""))
...
**我的反思（針對 LLM 行為模式，不依賴特定字句）**：

1. **LLM 答得好的部分（內容層）**：當 prompt 提供具體 km²、閾值、限制聲明時，模型會自然採用我給的 4-class 信心分級邏輯做撤離優先順序，並把 High Confidence 與 SAR Only 分層處理。對「DEM 失準」「雲遮蔽限制」這類我在 prompt 中**已經寫出**的限制，模型會具體展開為可操作建議。這驗證了「**evidence-based prompt 直接決定 LLM 輸出品質**」— 量化證據密度 ↑ → 回覆貼合實況 ↑。

2. **LLM 仍籠統的部分（地理層）**：Gemini 對「**通用資料產品名稱**」（如商業衛星、InSAR、UAV、地面感測器）相當具體 — 因為這是訓練資料涵蓋的「全球通用知識」；但對**花蓮縣本地名詞**（具體鄉鎮、避難所、橋梁、道路）則完全不能下手，因為它沒有縣府 GIS 圖層。實戰時必須把本地 GIS 資料以表格或 GeoJSON 摘要塞進 prompt 才能拿到地名級別的建議。

3. **LLM 在「不確定性表述」上的進步**：模型沒有把 0.11 km² High Conf 包裝成絕對結論，反而清楚標註「需要進一步光學/UAV 確認」。這是 Gemini 2.5 系列在校準（calibration）層面的進步 — 比早期 LLM 動輒「整張地圖紅色」要負責得多。

4. **對 ARIA v7.0 的啟示**：Gemini 在這次任務的角色是**翻譯器** — 把 km²、閾值、像素統計轉換成可讀決策語言。它不是替代決策者，而是**降低從「資料」到「敘事」的人力成本**。實際下令仍須由現場指揮官交叉比對 (a) NCDR 災情通報 (b) 警消實地回報 (c) CWB 降雨即時資料。"""))
